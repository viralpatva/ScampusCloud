    ...
    $.fn[pluginName] = function (options) {
        ...
        var Plugin = {
            ...
            init: function (options) {
                ...
                Plugin.setModifiedHeadTitle(datatable.tableHead);
            ...
            setupDOM: function (table) {
                // set table classes
                $(table).find('> thead').addClass(pfx + 'datatable-head rounded').css('background-color', '#3699FF');
                ...
                $(table).find('tr > th').each(function (i, th) {
                    $(th).addClass(pfx + 'datatable-cell');
                    if ($(th).hasClass(pfx + 'hide')) {
                        $(th).addClass(pfx + 'd-none');
                    }

                    if ($(th).hasClass(pfx + 'export')) {
                        $(th).addClass(pfx + 'export');
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-left')) {
                        $(th).addClass(pfx + 'datatable-cell-left');
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-right')) {
                        $(th).addClass(pfx + 'datatable-cell-right');
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-center')) {
                        $(th).addClass(pfx + 'datatable-cell-center');
                    }
                    if ($(th).find('span').length === 0) {
                        $(th).wrapInner($('<span/>').css('width', Plugin.cellOffset));
                    }
                });
                $(table).find('tr > td').each(function (i, td) {
                    $(td).addClass(pfx + 'datatable-cell');
                    if ($(td).hasClass(pfx + 'hide')) {
                        $(td).addClass(pfx + 'd-none');
                    }
                    if ($(td).hasClass(pfx + 'export')) {
                        $(td).addClass(pfx + 'export');
                    }
            ...
            setHeadTitle: function (tablePart) {
                ...
                $.each(columns, function (i, column) {
                    ...
                    // override visibility for table th
                    if ($(th).hasClass(pfx + 'hide')) {
                        $(th).addClass(pfx + 'd-none');
                    }
			 ...
			 * Set column title from options.columns settings
			 */
            setModifiedHeadTitle: function (tablePart) {
                if (typeof tablePart === 'undefined') tablePart = datatable.tableHead;
                tablePart = $(tablePart)[0];
                var columns = options.columns;
                var row = tablePart.getElementsByTagName('tr')[0];
                var ths = tablePart.getElementsByTagName('th');
                if (typeof row === 'undefined') {
                    row = document.createElement('tr');
                    tablePart.appendChild(row);
                }
            ...
            insertData: function () {
                ...
                // prepare array for hidden class containing column
                var hiddencolumnsArray = new Array();
                var leftaligncolumnsArray = new Array();
                var rightaligncolumnsArray = new Array();
                var centeraligncolumnsArray = new Array();
                var exportcolumnsArray = new Array();
                $(datatable.tableHead).find('tr > th').each(function (i, th) {
                    if ($(th).hasClass(pfx + 'hide')) {
                        hiddencolumnsArray.push($(th).text());
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-left')) {
                        leftaligncolumnsArray.push($(th).text());
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-right')) {
                        rightaligncolumnsArray.push($(th).text());
                    }
                    if ($(th).hasClass(pfx + 'datatable-cell-center')) {
                        centeraligncolumnsArray.push($(th).text());
                    }
                    if ($(th).hasClass(pfx + 'export')) {
                        exportcolumnsArray.push($(th).text());
                    }

                });

                //// if custom paging applied then tablebody should be rendered as per page size
                if ($(".custom-paging").length > 0) {
                    $.each(datatable.dataSet.prevObject, function (rowIndex, row) {
                        ...
                        for (var a = 0; a < colLength; a += 1) {
                            ...
                            // apply hidden class for matching column
                            if (jQuery.inArray(column.field, hiddencolumnsArray) != -1) {
                                classes.push('d-none');
                            }

                            // apply alignment class for matching column
                            if (jQuery.inArray(column.field, leftaligncolumnsArray) != -1) {
                                classes.push('datatable-cell-left');
                            }
                            if (jQuery.inArray(column.field, rightaligncolumnsArray) != -1) {
                                classes.push('datatable-cell-right');
                            }
                            if (jQuery.inArray(column.field, centeraligncolumnsArray) != -1) {
                                classes.push('datatable-cell-center');
                            }

                            // apply export class for matching column
                            if (jQuery.inArray(column.field, exportcolumnsArray) != -1) {
                                classes.push('export');
                            }
            ...
            paging: function (meta, callback) {
                ...
                /**
                 * override paging footer for specific page listing i.e Manage Client
                 */
                if ($(".custom-paging").length > 0) {
                    //default paging div rendered which is hide.
                    $('.datatable-pager').first().hide();
                }
                return pg;
            },
            ...
            sortCallback: function (data, sort, column) {
                ...
                var format = column['format'] || 'MM/DD/YYYY hh:mm a';
                var field = column['field'];

                return $(data).sort(function (a, b) {
                    var aField = a[field];
                    var bField = b[field];
                    if ($.isNumeric(aField) && $.isNumeric(bField)) {
                        type = 'number';
                    }
                    else if (!isNaN(Date.parse(aField)) && !isNaN(Date.parse(bField))) {
                        type = 'date';
                    }
                    switch (type) {
                        case 'date':
            ...
            autoHide: function () {
                ...
                var toggleHiddenColumns = function (e) {
                    ...
                    if (!$(detailRow).hasClass(pfx + 'datatable-row-detail')) {
                        ...
                        var $currentHtml = $('<div>').append(detailSubTable.html());

                        // .find() will now be able to search through all the markup you added
                        $currentHtml.find('.d-none').removeClass(pfx + 'd-none');

                        // .html() will now give you back all the markup (though modified)
                        $(detailRowTd).append($currentHtml);
                ...
                setTimeout(function () {
                    ...
                    $(datatable.tableBody).each(function () {
                        // if any column contains class 'd-none' then set hiddenExist to true
                        $(datatable.table).find('.' + pfx + 'datatable-row').each(function (i) {
                            if ($(this).find('td').hasClass('d-none')) {
                                $(this).find('td').addClass(pfx + 'hide');
                                hiddenExist = true;
                            }
                        });
                        var recursive = 0;
                    ...
                    if (hiddenExist) {
                        ...
                        // add empty column to the table row
                        $(datatable.tableBody).find('.' + pfx + 'datatable-row').each(function () {
                            $(this).prepend('<td class="' + pfx + 'datatable-cell ' + pfx + 'datatable-toggle-detail"><span></span></td>');
                        });
                    }
                });
            ...
            rowEvenOdd: function () {
                ...
                if ($(datatable.wrap).hasClass(pfx + 'datatable-subtable')) {
                    $(datatable.tableBody).find('.' + pfx + 'datatable-row:not(.' + pfx + 'datatable-row-detail):even').addClass(pfx + 'datatable-row-even rounded').css('background-color', '#eef0f8');
                } else {
                    $(datatable.tableBody).find('.' + pfx + 'datatable-row:nth-child(even)').addClass(pfx + 'datatable-row-even rounded').css('background-color', '#eef0f8');
                }
            },
...
var KTLayoutSearch = function () {
    ...
    var _requestTimeout = 300; // ajax request fire timeout in milliseconds
    ...
    var _processSearch = function () {
        ...
        setTimeout(function () {
            $.ajax({
                ...
                url: '/en-US/Home/GlobalSearchResult',
                data: {
                    query: _query
                },
                dataType: 'html',
    ...
    return {
        init: function (id) {
            ...
            KTUtil.addEvent(_input, 'keydown', _handleSearch);
