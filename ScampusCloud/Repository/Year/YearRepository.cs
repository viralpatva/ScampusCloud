...
namespace ScampusCloud.Repository.Year
{
	public class YearRepository
	{
		private GeneralMethods objgm = new GeneralMethods();
		public YearModel AddEdit_Year(YearModel _YearModel)
		{
			try
			{
				QueryBuilder objQueryBuilder = new QueryBuilder
				{
					TableName = _YearModel.GetType().Name,
		...
		public List<YearModel> GetYearList(string searchtxt = "", int page = 1, int pagesize = 10, string CompanyId = null)
		{
			try
			{
				QueryBuilder objQueryBuilder = new QueryBuilder
				{
					TableName = "Tbl_Mstr_Year",
					StoredProcedureName = @"SP_GetYearData",
					SetQueryType = QueryBuilder.QueryType.SELECT
				};
				...
				return objgm.GetListUsingSp<YearModel>(objQueryBuilder);
			}
			catch (Exception ex)
			{
				ErrorLogger.WriteToErrorLog(ex.Message, ex.StackTrace, ex.InnerException != null ? ex.InnerException.ToString() : string.Empty, this.GetType().Name + " : " + MethodBase.GetCurrentMethod().Name);
		...
		public string GetAllCount(string searchtxt = "", string CompanyId = null)
		{
			try
			{
				...
				{
					TableName = "Tbl_Mstr_Year",
					StoredProcedureName = @"SP_GetYearData_Count",
					SetQueryType = QueryBuilder.QueryType.SELECT,
				};
				objQueryBuilder.AddFieldValue("@Search", searchtxt, DataTypes.Text, false);
				objQueryBuilder.AddFieldValue("@CompanyId", CompanyId, DataTypes.Text, false);
		...
		public DataTable GetYearData_Export(string searchtxt = "", string CompanyId = null)
		{
			try
			{
				QueryBuilder objQueryBuilder = new QueryBuilder
				{
					TableName = "Year",
					StoredProcedureName = @"SP_DownloadYearData_Export",
					SetQueryType = QueryBuilder.QueryType.SELECT
				};
				objQueryBuilder.AddFieldValue("@Search", searchtxt, DataTypes.Text, false);
				objQueryBuilder.AddFieldValue("@CompanyId", CompanyId, DataTypes.Text, false);
