...
namespace ScampusCloud.Controllers
{
    ...
    public class CollegeController : Controller
    {
        ...
        CollegeModel _CollegeModel = new CollegeModel();
        #endregion
        #region CTOR
        public CollegeController()
        {
            _CollegeRepository = new CollegeRepository();
        }
        #endregion
        ...
        public ActionResult College()
        {
            ...
            string searchtxt = "NA";
            int totals = Convert.ToInt32(_CollegeRepository.GetAllCount(searchtxt, SessionManager.CompanyId.ToString()));
            ViewData["totalrecords"] = totals;
            return View();
        }
        public ActionResult AddEditCollege(string ID = "")
        {
            try
            {

                _CollegeModel.CreatedBy = SessionManager.UserId;
                _CollegeModel.ModifiedBy = SessionManager.UserId;
                _CollegeModel.CompanyId = SessionManager.CompanyId;
                
                if (!string.IsNullOrEmpty(ID) && ID != "0")
                {
                    #region Get Entity by id
                    _CollegeModel.ActionType = "Edit";
                    _CollegeModel.Id = Convert.ToInt32(ID);
                    _CollegeModel = _CollegeRepository.AddEdit_College(_CollegeModel);
                    if (_CollegeModel != null)
                    {
                        _CollegeModel.IsEdit = true;
                        SessionManager.Code = _CollegeModel.Code;
                    }
                    else
                    {
                        _CollegeModel = new CollegeModel();
                        ViewBag.NoRecordExist = true;
                        _CollegeModel.Response_Message = "No record found";
                ...
                {
                    return RedirectToAction("AddEditCollege");
                }
                else
                {
                    _CollegeModel.IsEdit = false;
                    _CollegeModel.IsActive = true;
                    SessionManager.Code = null;
                }
                _CollegeModel.lstCampus = BindDropdown(SessionManager.CompanyId.ToString());
                return View(_CollegeModel);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public ActionResult AddEditCollege(CollegeModel _CollegeModel, string saveAndExit = "")
        {
            if (_CollegeModel.Id > 0)
            {
                _CollegeModel.ActionType = "Update";
            }
            else
            {
                _CollegeModel.ActionType = "Insert";
            }
            var officemaster = _CollegeRepository.AddEdit_College(_CollegeModel);
            if (!string.IsNullOrEmpty(saveAndExit))
            {
                return RedirectToAction("College", "College");
            }
            else if (_CollegeModel.IsEdit == true)
            {
                return RedirectToAction("AddEditCollege", new { ID = _CollegeModel.Id });
            }
            else
            {
                return RedirectToAction("AddEditCollege");
        ...
        public ActionResult CollegeList(int page = 1, int pagesize = 10, string searchtxt = "")
        {
            try
            {
                searchtxt = string.IsNullOrEmpty(searchtxt) ? "" : searchtxt;
                int totals = Convert.ToInt32(_CollegeRepository.GetAllCount(searchtxt, SessionManager.CompanyId.ToString()));
                var lstCountries = _CollegeRepository.GetCollegeList(searchtxt, page, pagesize, SessionManager.CompanyId.ToString());
                ...
                if (lstCountries.Count > 0)
                {
                    ...
                    {
                        string DeleteConfirmationEvent = "DeleteConfirmation('" + item.Id + "','College','College','Delete')";
                        strHTML.Append("<tr>");
                        strHTML.Append("<td>" + item.Code + "</td>");
                        strHTML.Append("<td>" + item.Name + "</td>");
                        strHTML.Append("<td>" + item.ScampusName + "</td>");
                        if (item.IsActive)
                        ...
                        strHTML.Append("<td>");
                        strHTML.Append("<a class='btn btn-sm btn-icon btn-lg-light btn-text-primary btn-hover-light-primary mr-3' href= '/College/AddEditCollege?ID=" + item.Id + "'><i class='flaticon-edit'></i></a>");
        ...
        public ActionResult CollegeListCount()
        {
            try
            {
        ...
        public ActionResult Delete(string Id)
        {
            try
            {
                _CollegeModel.ActionType = "Delete";
                _CollegeModel.Id = Convert.ToInt32(Id);
                var response = _CollegeRepository.AddEdit_College(_CollegeModel);
        ...
        public FileResult Export(string searchtxt = "")
        {
            ...
            try
            {
                dt = _CollegeRepository.GetCollegeData_Export(searchtxt, SessionManager.CompanyId.ToString());
                using (XLWorkbook wb = new XLWorkbook())
                {
                    ...
                    {
                        wb.SaveAs(stream);
                        return File(stream.ToArray(), "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "College.xlsx");
                    }
                }
            }
            catch (Exception ex)
        ...
        private List<SelectListItem> BindDropdown(string CompanyId)
        {
            ...
            drpList = _CollegeRepository.BindCampusDropDown(CompanyId);
            return drpList;
        }
        #endregion
    }
